...
in
{
  ...
  mkQuadletService =
    ...
    let
      ...
      healthCheckPort =
        if lib.hasAttr "httpPort" healthCheck && healthCheck.httpPort != null then
          healthCheck.httpPort
        else if lib.hasAttr "tcpPort" healthCheck && healthCheck.tcpPort != null then
          healthCheck.tcpPort
        else
          port;

      healthCheckCmd =
        if !healthCheck.enable then
          null
      ...
      watchdogSec =
        if enableWatchdog && healthCheck.enable then
          let
    ...
    in
    {
        ...
        containerConfig = lib.mkMerge [
          {
            ...
            environments =
              environments
              // (lib.optionalAttrs (containerUser != null && requiresPostgres) {
        ...
        unitConfig = lib.mkMerge [
          {
            ...
            Wants = [ "sops-nix.service" ] ++ lib.optional requiresPostgres "postgresql.service";
      ...
      # State directory
      systemd.tmpfiles.rules = [ "d /var/lib/${name} 0755 root root -" ] ++ tmpfilesRules;
    };
}
