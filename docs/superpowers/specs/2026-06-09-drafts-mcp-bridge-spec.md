...
| D7 | Write tools for autonomous VMs | **Deny all 9 write tools (incl. `drafts_run_action`) to BOTH VMs.** OpenClaw: filter shim (hard). Hermes: `tools.include` allowlist (default-deny). Host CC: full toolset. |
