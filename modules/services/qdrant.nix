...

{
  ...
  systemd.services.qdrant-certificate = {
    ...
    script = ''
      ...
      CERT_FILE="$CERT_DIR/qdrant.${hostPolicy.dnsName}.crt"
      KEY_FILE="$CERT_DIR/qdrant.${hostPolicy.dnsName}.key"
      ...
      echo "Creating bootstrap self-signed certificate for qdrant.${hostPolicy.dnsName}"
      ${pkgs.openssl}/bin/openssl req -x509 -newkey rsa:2048 \
        -keyout "$KEY_FILE" \
        -out "$CERT_FILE" \
        -days 365 \
        -nodes \
        -subj "/CN=qdrant.${hostPolicy.dnsName}" \
        -addext "subjectAltName=DNS:qdrant.${hostPolicy.dnsName}"
      ...
      echo "  sudo /etc/nixos/certs/renew-certificate.sh qdrant.${hostPolicy.dnsName} \\"
  ...
  services.nginx.virtualHosts."qdrant.${hostPolicy.dnsName}" = {
    forceSSL = true;
    sslCertificate = "/var/lib/nginx-certs/qdrant.${hostPolicy.dnsName}.crt";
    sslCertificateKey = "/var/lib/nginx-certs/qdrant.${hostPolicy.dnsName}.key";
