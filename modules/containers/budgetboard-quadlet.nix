...
in
{
  ...
  systemd.services.budgetboard-certificate = {
    ...
    #
    # This unit renews correctly (openssl -checkend 2592000 below, reissue under
    # 30 days) but until 2026-09-10 it had no timer and RemainAfterExit=true, so
    # it ran only at boot or on a definition change. This host is rebooted
    # deliberately -- uptime was 68 days -- and the renewal has to land INSIDE
    # the final 30 days, so there was often no trigger at all. Proof: the cert
    # still carried notBefore=Oct 30 2025, and this unit's last run (2026-07-03)
    # correctly skipped because ~119 days remained.
    #
    # With RemainAfterExit=true the unit stays `active` forever after its first
    # run, and `systemctl start` on an active oneshot is a NO-OP -- verified
    # 2026-09-10, ExecMainExitTimestamp did not move. So a timer alone would have
    # been worse than nothing: armed timer, "active" unit, script never running.
    serviceConfig = {
      Type = "oneshot";
  ...
  # Daily renewal check for the BudgetBoard certificate.
  #
  # The service is a no-op while the cert has more than 30 days left (its own
  # `openssl -checkend 2592000` guard), so firing weekly costs one openssl call and
  # issues nothing until renewal is genuinely due. That guard is what makes a
  ...
  # unconditionally every month, so listing budget.vulcan.lan there as well would
  # double-issue. Its header documents the exclusion deliberately.
  systemd.timers.budgetboard-certificate = {
    description = "Daily renewal check for the BudgetBoard TLS certificate";
    ...
    timerConfig = {
      ...
      Unit = "budgetboard-certificate.service";
    };
  };
}
