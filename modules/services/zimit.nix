...
RUNNER
  ...
  zimitJobRunner = pkgs.writeShellApplication {
    ...
    runtimeInputs = with pkgs; [ jq coreutils podman ];
    text = ''
      set -euo pipefail
...
in
{
  ...
  systemd.services.zimit-job-runner = {
    ...
    path = with pkgs; [ podman jq coreutils ];
  ...
  in {
    ...
    after = [ "network.target" "tank-Archives-ZIM.mount" ];
  ...
  networking.firewall.interfaces."lo".allowedTCPPorts = [ zimitPort kiwixPort ];
