...
2. POST the MCP `initialize` JSON-RPC request with `clientInfo: { name: "openclaw-hermes-smoke", version: "0.1.0" }` and `capabilities: {}`. Since the script is stdlib-only (no SDK import), hardcode the protocolVersion string the `mcp` SDK speaks at packaging time as the client's offer (a single constant near the top of the script — updated by hand when the SDK bumps). Parse the `protocolVersion` from the server's `initialize` *response* (delivered via the SSE channel) and use **the server's value** as the authoritative negotiated version for any subsequent messages — if the script can't honor the server's choice the probe's `_ok` will naturally become 0, which is the correct liveness signal.
