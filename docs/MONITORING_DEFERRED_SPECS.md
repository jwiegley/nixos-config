...
**`/etc/nixos` git drift** — `git status --porcelain` = **0 lines** right now. No gauge exists. `.nixos-build` is gitignored (commit `cf324d2`) and AIDE-excluded.
