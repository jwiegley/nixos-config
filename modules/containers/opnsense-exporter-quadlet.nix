...
#
# NOTE: Using a Python HTTP proxy to transform API responses as a workaround
...
# - The proxy (opnsense-api-transformer.nix) transforms boolean monitor_disable to string
#
# TO REMOVE THIS WORKAROUND when a fixed version is released
# (steps 2-6 live in modules/users/home-manager/opnsense-exporter.nix, not here):
#   1. Remove opnsense-api-transformer.nix and its import from quadlet.nix
...
#   5. Re-add the volume mounts for CA certificates (dropped in the Home Manager
#      migration; there is nothing commented out there to simply re-enable)
#   6. Re-add the SSL_CERT_FILE environment variable
...

{
  ...
  # Open firewall port on the lo and podman0 interfaces for container access
