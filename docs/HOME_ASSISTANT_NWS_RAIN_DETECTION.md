...
> **Status (2026-07-27):** These sensors **are deployed** on vulcan, but not by the
> procedure in the "Installation" section below. They live in
> `/var/lib/hass/packages/nws_rain.yaml` (17 entities — the 13 sensors and 4 binary
> sensors listed under "What You Get"), loaded through Home Assistant's *packages*
> mechanism, which the NixOS module enables with
> `homeassistant.packages = "!include_dir_named packages"` in
> `/etc/nixos/modules/services/home-assistant.nix`.
>
> **Do not follow Step 2 below on this host.** `/var/lib/hass/configuration.yaml` is
> generated from `services.home-assistant.config`, and because `configWritable = true`
> the module's `preStart` copies the generated file over it on *every* start — hand
> edits are silently lost on the next restart. The `template:` key is also already
> defined in Nix (the presence-detection binary sensors), so a second `template:` key
> in the same file would be invalid YAML anyway.
>
> To add or change sensors, either drop a YAML file into `/var/lib/hass/packages/`
> or add them to the module's `template` list. The leftover copy at
> `/var/lib/hass/nws-rain-sensors.yaml` (from the original 2025-10 install) is not
> referenced by anything and is inert.
...
- ✅ Integration enabled via the `"nws"` entry in the `extraComponents` list in
  `/etc/nixos/modules/services/home-assistant.nix` (line number omitted deliberately —
  it drifts; grep for `"nws"`)
...
> **Superseded on this host (2026-07-27)** — see the status note at the top of this
> document. `/var/lib/hass/configuration.yaml` is regenerated by the NixOS module on
> every Home Assistant start, so all three options below are overwritten. Use
> `/var/lib/hass/packages/` (as the live `nws_rain.yaml` does) instead. The options
> are kept for background and for use on non-NixOS installs.
...
- Rain sensor templates (repo copy): `/etc/nixos/config/home-assistant/nws-rain-sensors.yaml`
  — note (2026-07-27) this copy still reads `forecast[0].detailed_description`, which the
  NWS integration does not provide; the **live** version is
  `/var/lib/hass/packages/nws_rain.yaml`, which uses the `condition` attribute instead
- Rain automations: `/etc/nixos/config/home-assistant/nws-rain-automations.yaml`
  (14 automations; the 12-item summary above merges the enable/disable and
  set/clear pairs)
