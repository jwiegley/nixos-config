...

{
  ...
  # The house's real coordinates, feeding zone.home through !secret latitude /
  # !secret longitude in configuration.yaml.
  #
  # THESE LIVE IN SOPS RATHER THAN IN THIS FILE because this module is tracked in
  # git; a home address does not belong in plaintext in a repository, even a
  # private one.
  #
  # They were a HARDCODED PLACEHOLDER until 2026-09-01 -- generic Sacramento
  # city-centre coordinates written by the preStart heredoc below, under the
  # comment "Update with your actual coordinates if needed", which never was. The
  # house is ~9.3 km from that point and zone.home's radius is 200 m, so NO
  # GPS-sourced device_tracker could ever be inside the home zone: across a 29-day
  # recorder window not one ever reported 'home', including a stationary Mac Studio
  # that cannot leave the house. Everything that reported 'home' was router-sourced.
  #
  # That silently removed all redundancy from presence -- person.john_wiegley could
  # only ever be held home by the single flap-prone OPNsense router tracker -- and
  # is the root cause of three false alarm armings (2026-06-26, 2026-07-01 and
  # 2026-09-01, the last of which armed the house at 07:07 while both occupants
  # were asleep inside). See obr nixos-h2a.
  ...
  systemd.services.home-assistant = {
    ...
    preStart = ''
            ...
            # Generate secrets.yaml. Only elevation is inline -- it is not
            # location-identifying on its own. latitude/longitude come from SOPS and are
            # appended below.
            cat > /var/lib/hass/secrets.yaml << 'EOF'
      # Auto-generated by home-assistant.nix preStart. Do not edit by hand:
      # this file is rewritten on every start.
      elevation: 30
      EOF

            # Latitude/longitude are REQUIRED and are deliberately fail-closed. Unlike the
            # optional keys below, there is no sensible default: configuration.yaml
            # resolves zone.home through !secret latitude / !secret longitude, and the
            # previous behaviour -- silently writing placeholder city-centre coordinates --
            # put the home zone 9.3 km from the house for months without ever erroring.
            # Every GPS-based presence decision was wrong the entire time and nothing
            # surfaced it. A hard failure here is enormously preferable to starting again
            # with a misplaced home zone.
            LAT_FILE=${config.sops.secrets."home-assistant/latitude".path}
            LON_FILE=${config.sops.secrets."home-assistant/longitude".path}
            if [ ! -s "$LAT_FILE" ] || [ ! -s "$LON_FILE" ]; then
              echo "home-assistant preStart: SOPS secrets home-assistant/latitude and" >&2
              echo "  home-assistant/longitude must both exist and be non-empty." >&2
              echo "  zone.home resolves through them; refusing to start with a wrong home zone." >&2
              exit 1
            fi
            echo "latitude: $(cat "$LAT_FILE")" >> /var/lib/hass/secrets.yaml
            echo "longitude: $(cat "$LON_FILE")" >> /var/lib/hass/secrets.yaml
