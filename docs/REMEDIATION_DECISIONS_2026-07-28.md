...
|---|---|---|---|
| D1 | Discord bot token in history | **CLOSED — not the operator's app.** Verified absent from their Discord account. | Nothing to rotate. The fixture redaction (`8762657d`) is the complete fix. Discord's public API could not adjudicate this — `/applications/{id}/rpc` returns "Unknown Application" even for MEE6 and Dyno, so that endpoint is gated and proved nothing. |
...
| D5 | LiteLLM 55 GB spend-log bloat | **DONE.** Prompt bodies scrubbed, all rows kept. | See Part 3. Committed `5552318b`. Pre-scrub table retained until 2026-07-29 per operator caution, then drop. |
...
| M-94 | Identify the 21.75h Nagios divergence | **Unrecoverable** — the reconciler keeps no divergence detail. **Structural note worth retaining:** the mirror cannot detect the dead-rule class at all (Nagios evaluates the same broken expression, gets no data, lands OK/UNKNOWN, and the script skips UNKNOWN by design), so a green mirror must never be read as "no dead rules". |
